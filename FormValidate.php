...
//Validation class. Patterned on the codeigniter one. This doesn't work on nested arrays more than 1 level deep.
...
/*		'valid_emails'			=> "The %s field must contain all valid email addresses.",*/ /* comma-separated list */
...
/*		'regex_match'			=> "The %s field is not in the correct format.",*/ /* not publicised in CI as expressions are limited */
...
// ----------------------------------------------------------------------------
/*
	//experimental routine to make it a psuedo-singleton with only one instance
    private static $_instance;
	
    public static function instance($initial_data = array())
    {

		if(!self::$_instance)
		{
			self::$_instance = new self($initial_data);
		}
...
//TODO: error handling?
        //trigger_error('Call to non-existing method/function: '.$method, E_USER_ERROR);
...
//TODO: error handling for bad rules - could be flagged
			...
			//if a valid rule string add the whole rule to the array, else discard it.
			if($found) 
			{
				$this->rules[] = $rule;
			}
...
// ----------------------------------------------------------------------------
	...
	public function add_field_list ($field_list=array())
	{
		...
		{
			if(!is_array($field) || !isset($field['field'],$field['alias'],$field['rule_list'])) continue;//skip duff
...
// ----------------------------------------------------------------------------
	...
	private function report_error($routine_name,$field_name,$alias,$param)
	{
		...
		{
			//default message if nothing specific can be found
